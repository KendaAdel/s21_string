...
int what_is_speca(char *str, char *speca, state_of_speca *state,
                  va_list *arg_list);
int speca_is_s(char *str, char *speca, state_of_speca *state,
               va_list *arg_list);
...
int handle_speca_c(char *str, char *speca, state_of_speca *state,
                   va_list *arg_list);
int handle_speca_lc(char *str, char *speca, state_of_speca *state,
                    va_list *arg_list);
int handle_speca_s(char *str, char *speca, state_of_speca *state, char *stroka);
int handle_speca_ls(char *str, char *speca, state_of_speca *state,
                    va_list *arg_list);
// int main() {
//   setlocale(LC_ALL, "");
//   char stroka[110000];
...
//   sprintf(stroka2, "Хренька %c %d\n%5ls %d\n", 'Z', 2, L"жопаропа", 1);
//   printf("%s\n", stroka2);

//   s21_sprintf(stroka, "Хренька %c %d\n%+5ls %d\n", 'Z', 2, L"жопаропа", 1);
//   printf("%s", stroka);
//   return 0;
// }
...
int what_is_speca(char *str, char *speca, state_of_speca *state,
  ...
  switch (speca[len - 1]) {
    case 's':
      result = speca_is_s(str, speca, state, arg_list);
      break;
    case 'c':
...
int speca_is_s(char *str, char *speca, state_of_speca *state,
  ...
  switch (state->longshort) {
    case 'l':
      handle_speca_ls(str, speca, state, arg_list);
      break;
    case 0:
      char *stroka = va_arg(*arg_list, char *);
      handle_speca_s(str, speca, state, stroka);
      break;
    default:
      break;
  }
...
int handle_speca_s(char *str, char *speca, state_of_speca *state,
                   char *stroka) {
  int result = 0;

  int len = (int)strlen(stroka);
  if (!state->minus && state->is_width)
    for (int i = 1; i < state->width - (len - state->precision) - 1; i++)
      strcat(str, " ");
  state->is_precision ? strncat(str, stroka, state->precision)
                      : strcat(str, stroka);
  if (state->minus && state->is_width)
    for (int i = 1; i < state->width - (len - state->precision) - 1; i++)
      strcat(str, " ");
...
int handle_speca_ls(char *str, char *speca, state_of_speca *state,
                    va_list *arg_list) {
  int result = 0;
  // Получаем широкую строку из va_list
  wchar_t *wstroka = va_arg(*arg_list, wchar_t *);
  // Определяем размер буфера для многобайтовой строки
  size_t bufferSize = wcstombs(NULL, wstroka, 0) + 1;
  int wlen = (int)bufferSize - 1;
  // Выделяем память для многобайтовой строки
  char *mnogobyte_str = (char *)malloc(bufferSize);
  // Преобразуем широкую строку в многобайтовую
  const wchar_t *src =
      wstroka;         // Указатель на текущую позицию в широкой строке
  mbstate_t ps = {0};  // Состояние преобразования
  size_t transformation = wcsrtombs(mnogobyte_str, &src, bufferSize - 1, &ps);
  // Обрезаем строку, если precision установлена
  if (state->precision > 0 && state->precision < (int)transformation) {
    // Обрезаем строку с учётом многобайтовой кодировки
    int pos = 0;
    int byteCount = 0;
    wlen = 0;

    while (byteCount < state->precision && mnogobyte_str[pos] != '\0') {
      // Определяем длину текущего символа
      int charLen = mblen(&mnogobyte_str[pos], bufferSize - pos);
      if (charLen <= 0) {
        // Некорректный символ
        break;
      }
      if (byteCount + charLen > state->precision) {
        // Превышение precision, останавливаемся
        break;
      }
      byteCount += charLen;
      pos += charLen;
    ...
    mnogobyte_str[pos] = '\0';  // Обрезаем строку
  }

  if (!state->minus && state->is_width) {
    for (int i = 1; i < state->width - wlen - 1; i++) {
      strcat(str, " ");
    }
  }
  strcat(str, mnogobyte_str);
  if (state->minus && state->is_width)
    for (int i = 1; i < state->width - wlen - 1; i++) strcat(str, " ");
  free(mnogobyte_str);
  return result;
}
...
int speca_is_c(char *str, char *speca, state_of_speca *state,
  ...
  switch (state->longshort) {
    case 'l':
      handle_speca_lc(str, speca, state, arg_list);
      break;
    case 0:
      handle_speca_c(str, speca, state, arg_list);
      break;
    default:
      break;
  }
...
int handle_speca_c(char *str, char *speca, state_of_speca *state,
  ...
  if (!state->minus && state->is_width)
    for (size_t i = 1; i < state->width; i++) strcat(str, " ");
...
int handle_speca_lc(char *str, char *speca, state_of_speca *state,
  ...
  if (!state->minus && state->is_width)
    for (size_t i = 1; i < state->width - 1; i++) strcat(str, " ");
  if (result > 0) {
    buffch[result] = '\0';  // Добавляем нулевой терминаторыек
    strcat(str, buffch);
    result = 0;
  } else
    result = 1;
